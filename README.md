...
The goal of our final project is to address these issues by creating a capacitive touch based AAC system that can be used to train dogs to communicate and track their learning progress. More specifically, we plan to use Adafruit’s capacitive touch sensor breakout MPR121 - STEMMA QT) to create 12 capacitive touch based “buttons” that will each play a different word when touched, track the number of times each button is pressed and display a graph of the number of times each word is pressed in total and per day.  We will connect the sensor wirelessly to the server and use a python library called gTTS to convert text-to-speech to generate the mp3 files.
